...
def read_point(point_encoding):
    coordinates = point_encoding.split(' ')
    assert len(coordinates) == 2
    return Point(float(coordinates[0]), float(coordinates[1]))

def read_line(line_encoding):
    coordinates = line_encoding.split(' ')
    assert len(coordinates) == 4
    a = Point(float(coordinates[0]), float(coordinates[1]))
    b = Point(float(coordinates[2]), float(coordinates[3]))
    return Line(a, b)
...
class Point:
    ...
    def distance(self, point):
        """
        The function to calculate Euclidean distance between this and given point.
        Arguments:
...
class Line:
    """
    The simple line segment between two points. Used to represent maze walls.
    """
