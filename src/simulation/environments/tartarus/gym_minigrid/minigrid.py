...
class WorldObj:
    ...
    def can_push(self):
        """Can the agent push this object?"""
        return False
...
class MiniGridEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Move forward
            ...
            if fwd_cell is not None and fwd_cell.type == 'box':
                push_valid, push_to_pos = push_is_valid(fwd_pos)
                if push_valid:
                    # push box to new position
                    self.grid.set(*push_to_pos, fwd_cell)
                    fwd_cell.cur_pos = push_to_pos
    ...
    def push_is_valid(self, fwd_pos):
        push_to_pos = fwd_pos + self.dir_vec
        push_to_pos_cell = self.grid.get(*push_to_pos)
        if push_to_pos_cell is None:
            return True, push_to_pos
