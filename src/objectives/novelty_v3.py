...
class NoveltyItem:
    ...
    def __str__(self):
        ...
        return "%s: id: %d, at generation: %d, fitness: %f, novelty: %f\tdata: %s" % \
               (self.__class__.__name__, self.genomeId, self.generation,
                self.fitness, self.novelty, self.data)
...
class NoveltyArchive:
    ...
    def evaluate_individual_novelty(self, genome, genomes, n_items_map, only_fitness=False):
        ...
        if only_fitness:
            ...
            if result > self.novelty_threshold or len(self.archive) < ArchiveSeedAmount:
    ...
    def _map_novelty(self, item):
        ...
        """
        distances = [None] * len(self.archive)
        for i, n in enumerate(self.archive):
